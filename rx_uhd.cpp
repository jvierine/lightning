...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    bool nolocktest             = vm.count("nolock") > 0;        
    ...
    {
      // no internal gpsdo, we just have to rely on the external
      const time_t cpu_time = std::time(nullptr);
      usrp->set_clock_source("external");
      usrp->set_time_source("external");
      printf("using cpu time %d\n",cpu_time);
      usrp->set_time_next_pps(uhd::time_spec_t(cpu_time+1));
